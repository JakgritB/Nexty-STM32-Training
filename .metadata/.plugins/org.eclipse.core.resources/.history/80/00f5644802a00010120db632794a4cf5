...
// ============================================
// แปลงค่า ADC เป็นเกียร์ (Hard Code แบบชัดเจน)
// ============================================
uint8_t adc_to_gear(uint16_t adc_value) {
	// แบ่งช่วง ADC 0-4095 เป็น 6 เกียร์
	if (adc_value >= 3415 && adc_value <= 4095) {
		return GEAR_N;  // ช่วง 3415-4095
	} else if (adc_value >= 2733 && adc_value <= 3414) {
		return GEAR_1;  // ช่วง 2733-3414
	} else if (adc_value >= 2050 && adc_value <= 2732) {
		return GEAR_2;  // ช่วง 2050-2732
	} else if (adc_value >= 1367 && adc_value <= 2049) {
		return GEAR_3;  // ช่วง 1367-2049
	} else if (adc_value >= 684 && adc_value <= 1366) {
		return GEAR_4;  // ช่วง 684-1366
	} else {
		return GEAR_5;  // ช่วง 0-683
	}
}
...
int main(void) {
	...
	GPIOA->MODER |= (0b11 << GPIO_MODER_MODER4_Pos);  // PA4: Gear
